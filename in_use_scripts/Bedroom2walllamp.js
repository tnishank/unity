#pragma strict
private var id_relay="ARE09";

private var state = 0;
public var BED2wall: GameObject;
...
function Start()
{
	...
	else
	{
    		...
    		if (res2["value"].AsInt == 0){
    			state=0;
    			BED2wall.gameObject.SetActive(false);
    			Debug.Log("device off");
    			}
    		else {
    		  state=1;
    		  BED2wall.gameObject.SetActive(true);
...
function Update(){
	if (Input.touchCount==1 && Input.GetTouch(0).phase == TouchPhase.Began){
				...
				{
					if (hit.collider.gameObject.name == "BD_SMALL_LAMP_1_BOX")
					{
						devicetouched();	
					}
				}
