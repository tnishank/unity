#pragma strict
private var id_relay="ARE06";

private var state = 0;
public var appliance_led: GameObject;
...
function Start()
{
	...
	if (request.error != null)
	{
    		...
    		appliance_led.gameObject.SetActive(false);
    		state = 0;
	}
	else
	{
    		...
    		if (res2["value"].AsInt == 0){
    			state=0;
    			appliance_led.gameObject.renderer.material.color = Color.red;
    			Debug.Log("device off");
    			}
    		else {
    		  state=1;
    		  appliance_led.gameObject.renderer.material.color = Color.green;
...
function Update(){
	if (Input.touchCount==1){
				...
				{
					if (hit.collider.gameObject.name == "SPEAKER_LEFT_TOUCH_BOX" || hit.collider.gameObject.name == "SPEAKER_RIGHT_TOUCH_BOX")
					{
						devicetouched();	
					}
				}
