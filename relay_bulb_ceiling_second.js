...
private var state = 0;
public var ItemLed4: GameObject;
...
function Start()
{
	...
	else
	{
    		...
    		if (res2["value"].AsInt == 0){
    			state=0;
    			ItemLed4.gameObject.SetActive(false);
    			Debug.Log("device off");
    			}
    		else {
    		  state=1;
    		  ItemLed4.gameObject.SetActive(true);
...
function Update(){
	if (Input.touchCount==1 && Input.GetTouch(0).phase == TouchPhase.Began){
				...
				{
					if (hit.collider.gameObject.name == "Cube_light_ceiling_two")
					{
						/*if (state==0){
							state=1;
							ItemLed4.gameObject.SetActive(true);
