...
private var state = 0;
public var Bath1light: GameObject;
...
function Start()
{
	...
	else
	{
    		...
    		if (res2["value"].AsInt == 0){
    			state=0;
    			Bath1light.gameObject.SetActive(false);
    			Debug.Log("device off");
    			}
    		else {
    		  state=1;
    		  Bath1light.gameObject.SetActive(true);
...
function Update(){
	if (Input.touchCount==1 && Input.GetTouch(0).phase == TouchPhase.Began){
				...
				{
					if (hit.collider.gameObject.name == "WALL_LAMP_TOUCH")
					{
						devicetouched();	
					}
				}
